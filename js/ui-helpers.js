...
function returnToHub() {
    ...
    if (characterId && typeof showCharacterDetail === 'function') {
        const prevLevel = window.currentState?._prevLevel ?? null;
        const prevXP    = window.currentState?._prevXP    ?? null;
        if (window.currentState) {
            window.currentState._prevLevel = null;
            window.currentState._prevXP    = null;
        }
        showCharacterDetail(characterId, { prevLevel, prevXP });
    } else {
        showScreen('roster');
    }
}
